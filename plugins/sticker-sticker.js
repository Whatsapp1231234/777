...
if (new Date - user.lastmiming < 10000) return await conn.reply(m.chat, `*ПОДОЖДИТЕ НЕСКОЛЬКО МИНУТ, ЧТОБЫ ИСПОЛЬЗОВАТЬ ДРУГУЮ КОМАНДУ*`,  m)
  try {
  	
    let q = m.quoted ? m.quoted : m
    let mime = (q.msg || q).mimetype || q.mediaType || ''
    if (/webp|image|video/g.test(mime)) {
      if (/video/g.test(mime)) if ((q.msg || q).seconds > 11) return m.reply('╰⊱⚠️⊱ *ВНИМАНИЕ* ⊱⚠️⊱╮\n\nВИДЕО НЕ ДОЛНО ПРИВЫШАТЬ 7 СЕКУНД ДЛЯ СОЗДАНИЯ СТИКЕРА')
      ...
      await conn.reply(m.chat, `${eg}⏳ *СОЗДАНИЕ СТИКЕРА, МОМЕНТ...* 🐈`, m)
        if (!stiker) {
    ...
    else throw "╰⊱❗️⊱ *НЕ ПРАВИЛЬНО ИСПОЛЬЗУЕТЕ КОМАНДУ* ⊱❗️⊱╮\n\nОТВЕТТЕ КОМАНДОЙ НА ВЫДЕЛЕВ КАРТИНКУ ИЛИ ВИДЕО"
  }
